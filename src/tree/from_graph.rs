...
mod tests {
    ...
    #[test]
    fn p1() {
        let graph = from_tree(read("*", None).unwrap()).unwrap();
    ...
    #[test]
    fn methane() {
        let graph = from_tree(read("C", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn methane_ammonia_hydrate() {
        let graph = from_tree(read("C.N.O", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    #[test]
    fn methanol() {
        let graph = from_tree(read("CO", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    #[test]
    fn propanol() {
        let graph = from_tree(read("CCO", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn propanal_explicit_bonds() {
        let graph = from_tree(read("C-C=O", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn propanol_branched() {
        let graph = from_tree(read("C(O)CC", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn trihalo_methane_s3() {
        let graph = from_tree(read("C(F)(Cl)Br", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    #[test]
    fn fluoroethanol() {
        let graph = from_tree(read("C(CF)O", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    #[test]
    fn oxirane() {
        let graph = from_tree(read("C1CO1", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn oxirane_left_single() {
        let graph = from_tree(read("C-1CO1", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn oxirane_right_single() {
        let graph = from_tree(read("C1CO-1", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn oxirane_left_right_single() {
        let graph = from_tree(read("C-1CO-1", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    #[test]
    fn bicyclobutane() {
        let graph = from_tree(read("C12CC1C2", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn tetrahalomethane_stereocentric() {
        let graph = from_tree(read("[C@](F)(Cl)(Br)I", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn tetrahalomethane_non_stereocentric() {
        let graph = from_tree(read("F[C@](Cl)(Br)I", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn trihalomethane_stereocentric() {
        let graph = from_tree(read("[C@H](F)(Cl)Br", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn trihalomethane_hydrate_stereocentric() {
        let graph = from_tree(read("O.[C@H](F)(Cl)Br", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
    ...
    fn trihalomethane_non_stereocentric() {
        let graph = from_tree(read("F[C@H](Cl)Br", None).unwrap()).unwrap();
        let tree = from_graph(graph).unwrap();
